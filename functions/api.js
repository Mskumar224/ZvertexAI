...
const requiredEnv = [
  'MONGODB_URI', 'EMAIL_USER', 'EMAIL_PASS', 'JWT_SECRET', 'FRONTEND_URL',
  'RAPIDAPI_KEY', 'CLOUDINARY_CLOUD_NAME', 'CLOUDINARY_API_KEY', 'CLOUDINARY_API_SECRET'
];
...
// Define all routes
app.get('/health', (req, res) => res.status(200).json({ message: 'Server running', dbConnected: true }));

app.post('/signup-initial', async (req, res) => {
  const { email, password, subscription, phone } = req.body;
  try {
    if (!email || !password || !subscription) return res.status(400).json({ message: 'Email, password, and subscription are required' });
    if (!['Student', 'Vendor/Recruiter', 'Business'].includes(subscription)) return res.status(400).json({ message: 'Invalid subscription type' });

    const existingUser = await User.findOne({ email });
    if (existingUser) {
      if (existingUser.paymentCompleted) return res.status(409).json({ message: 'Email already registered with a paid subscription' });
...
app.post('/upload-resume', upload, async (req, res) => {
  try {
...
app.post('/select-companies', async (req, res) => {
